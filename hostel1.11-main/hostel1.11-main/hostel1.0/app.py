...
class FeeCutApplication(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    student_id = db.Column(db.Integer, db.ForeignKey('student.id'), nullable=False)
    start_date = db.Column(db.Date, nullable=False)
    end_date = db.Column(db.Date, nullable=False)
    total_absent_days = db.Column(db.Integer, nullable=False)
    absence_percentage = db.Column(db.Float, nullable=True)  # Add this line
    reason = db.Column(db.Text, nullable=False)
    status = db.Column(db.String(20), default='pending')  # pending, approved, rejected
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
    
    # Relationship to link to Student model
    student = db.relationship('Student', backref=db.backref('fee_cut_applications', lazy=True))
class FeeCutToken(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    student_id = db.Column(db.Integer, db.ForeignKey('student.id'), nullable=False)
    token = db.Column(db.String(64), unique=True, nullable=False)
    fee_cut_application_id = db.Column(db.Integer, db.ForeignKey('fee_cut_application.id'), nullable=False)
    reduction_amount = db.Column(db.Float, nullable=False)
    is_used = db.Column(db.Boolean, default=False, nullable=False)
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
    expires_at = db.Column(db.DateTime, nullable=False)
    absence_percentage = db.Column(db.Float, nullable=True)
    status = db.Column(db.String(20), default='pending')
    student = db.relationship('Student', backref='fee_cut_tokens')
    fee_cut_application = db.relationship('FeeCutApplication', backref='fee_cut_tokens')
...
def payment():
    ...
    if 'user_id' in session:
        ...
        if student:
            # Check for unused, approved fee cut tokens
            fee_cut_token = FeeCutToken.query.filter(
                FeeCutToken.student_id == student.id,
                FeeCutToken.is_used == False,
                FeeCutToken.status == 'approved',  # Explicitly check for approved status
                FeeCutToken.expires_at > datetime.utcnow()
            ).first()
            
            # If approved fee cut token exists, apply the reduction
            if fee_cut_token:
                # Log the fee cut token details
                logger.info(f"Fee cut token found for student {student.name}")
                logger.info(f"Token reduction amount: ₹{fee_cut_token.reduction_amount}")
                
                # Apply the reduction
                discount += fee_cut_token.reduction_amount
            
            # Existing absence-based discount logic
            ...
            # Calculate additional discount: ₹20 for every 3 consecutive absences
    ...
    # Log payment details
    logger.info(f"Payment calculation for student: {student.name if 'student' in locals() else 'Unknown'}")
    logger.info(f"Original amount: ₹{base_amount}")
    ...
    try:
        ...
        if 'user_id' in session:
            ...
            if student:
                ...
                # Check for unused fee cut token
                fee_cut_token = FeeCutToken.query.filter(
                    FeeCutToken.student_id == student.id,
                    FeeCutToken.is_used == False,
                    FeeCutToken.expires_at > datetime.utcnow()
                ...
                # Mark fee cut token as used if it exists
                if fee_cut_token:
                    fee_cut_token.is_used = True
...
@app.route('/student/fee-cut-application', methods=['GET', 'POST'])
def student_fee_cut_application():
    if 'user_id' not in session or session['role'] != 'student':
        return redirect(url_for('login'))
    
    # Add error handling for student retrieval
    student = Student.query.filter_by(user_id=session['user_id']).first()
    
    # If no student is found, redirect to login or show an error
    if not student:
    ...
    if request.method == 'POST':
        try:
            ...
            if start_date > end_date:
                ...
                return redirect(url_for('student_fee_cut_application'))
            ...
            total_absent_days = len(absent_records)
            
            # Define attendance shortage threshold (e.g., more than 15% absences)
            total_days = (end_date - start_date).days + 1
            
            # Prevent division by zero
            absence_percentage = (total_absent_days / total_days * 100) if total_days > 0 else 0
            
            # Check if student already has a pending application
            existing_application = FeeCutApplication.query.filter(
                FeeCutApplication.student_id == student.id,
                FeeCutApplication.start_date <= end_date,
                FeeCutApplication.end_date >= start_date,
                FeeCutApplication.status.in_(['pending', 'approved'])
            ).first()
            
            if existing_application:
                flash('You already have a pending or approved application for this period', 'danger')
                return redirect(url_for('student_fee_cut_application'))
            
            # Create new fee cut application with initial status as 'pending'
            new_application = FeeCutApplication(
                student_id=student.id,
            ...
            flash('Fee cut application submitted successfully. It will be reviewed by the admin.', 'success')
            ...
            app.logger.error(f"Error in fee cut application: {str(e)}")
            flash('An unexpected error occurred. Please try again.', 'danger')
            return redirect(url_for('student_fee_cut_application'))
    
    # Get student's attendance records for the last 6 months
    ...
    return render_template('student/fee_cut_application.html', 
                           attendance_records=attendance_records)


@app.route('/admin/fee-cut-applications', methods=['GET'])
def admin_fee_cut_applications():
    if 'user_id' not in session or session['role'] != 'admin':
        return redirect(url_for('login'))
    
    # Get all pending fee cut applications with student details
    applications = db.session.query(
        ...
        FeeCutApplication.status == 'pending'
    ).order_by(
        FeeCutApplication.created_at.desc()
    ).all()
    
    return render_template('admin/fee_cut_applications.html', applications=applications)


@app.route('/admin/fee-cut-application/<int:application_id>/approve', methods=['POST'])
def approve_fee_cut_application(application_id):
    if 'user_id' not in session or session['role'] != 'admin':
        return redirect(url_for('login'))
    
    application = FeeCutApplication.query.get_or_404(application_id)
    
    # Ensure the application has not already been processed
    if application.status != 'pending':
        flash('This application has already been processed', 'warning')
        return redirect(url_for('admin_fee_cut_applications'))
    
    # Check both attendance shortage and admin approval conditions
    if application.absence_percentage >= 15:
        # Update application status to approved
        application.status = 'approved'
        
        # Calculate fee reduction (e.g., ₹20 per absent day)
        fee_reduction_per_day = 20
        total_reduction = application.total_absent_days * fee_reduction_per_day
        
        # Generate a unique token for the fee cut
        token = str(uuid.uuid4())
        
        # Create a new FeeCutToken only for approved applications
        fee_cut_token = FeeCutToken(
            student_id=application.student_id,
            token=token,
            fee_cut_application_id=application.id,
            reduction_amount=total_reduction,
            expires_at=datetime.utcnow() + timedelta(days=30),  # Token valid for 30 days
            status='approved'  # Explicitly set status
        )
        
        db.session.add(fee_cut_token)
        db.session.commit()
        
        flash('Fee cut application approved', 'success')
    else:
        # If attendance doesn't meet criteria, reject the application
        application.status = 'rejected'
        db.session.commit()
        
        flash('Fee cut application rejected due to insufficient attendance', 'warning')
    
    return redirect(url_for('admin_fee_cut_applications'))


@app.route('/admin/fee-cut-application/<int:application_id>/reject', methods=['POST'])
def reject_fee_cut_application(application_id):
    if 'user_id' not in session or session['role'] != 'admin':
        return redirect(url_for('login'))
    ...
    flash('Fee cut application rejected', 'warning')
    return redirect(url_for('admin_fee_cut_applications'))
